  ...
  return (
    <section id="coding-profiles" className="py-24 relative z-10">
      <div className="container mx-auto px-6 max-w-7xl 2xl:max-w-[1600px] 3xl:max-w-[1800px]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 2xl:gap-8 3xl:gap-12">
          {profiles.map((profile, idx) => (
            ...
            >
              <a href={profile.url} target="_blank" rel="noreferrer" className="block h-full">
                <SpotlightCard className="h-full p-8 2xl:p-10 3xl:p-12 flex flex-col items-center text-center backdrop-blur-xl group hover:border-[#FF4D4D]/50 transition-colors duration-500">
                  <div className="text-white/60 group-hover:text-[#FF4D4D] group-hover:scale-110 transition-all duration-500 mb-6">
                    {profile.icon}
                  </div>
                  <h3 className="text-2xl 3xl:text-3xl font-bold text-white mb-2">{profile.platform}</h3>
                  <div className="mt-4 px-6 py-2 3xl:px-8 3xl:py-3 rounded-full bg-white/5 border border-white/10 group-hover:bg-[#FF4D4D]/10 transition-colors">
                    <span className="text-sm 3xl:text-base text-white/60 mr-2">{profile.statLabel}:</span>
                    <span className="font-bold text-white 3xl:text-lg group-hover:text-[#FF4D4D]">
