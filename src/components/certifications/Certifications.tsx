        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 2xl:gap-8 3xl:gap-12">
          {certifications.map((cert, idx) => (
            <motion.div
              ...
              className="h-full"
            >
              <SpotlightCard className="h-full p-8 2xl:p-10 3xl:p-12 flex flex-col backdrop-blur-xl group cursor-pointer hover:border-white/20 transition-all duration-300">
                ...
                <h3 className="text-xl 2xl:text-2xl 3xl:text-3xl font-bold text-white mb-2">{cert.name}</h3>
                <p className="text-white/60 mb-4 3xl:text-lg">{cert.organization}</p>
                ...
                <div className="mt-auto pt-6 flex items-center justify-between border-t border-white/10">
                  <div className="flex flex-col">
                    ...
                    <span className="text-sm 3xl:text-base font-medium text-white/80">{cert.date}</span>
                  </div>
                  {cert.score && (
                    <div className="flex flex-col text-right">
                      <span className="text-xs text-white/40 uppercase tracking-wider">Score</span>
                      <span className="text-sm 3xl:text-base font-bold text-[#F9CB28]">{cert.score}</span>
                    </div>
                  )}
                </div>
              </SpotlightCard>
